...
bot = telebot.TeleBot('7637282666:AAH6fwKJxMKIs6FPixJs49AzkqGNKYMzrlM')
...
def answer(call):
    if call.data == 'v put':
        ...
        text = (
            ...
            "was the administrative and cultural center of the city. The square lost its historical name Sobornaya in "
            "1917. The main attraction of the square — the Peter and Paul Cathedral — was destroyed in 1935. What is "
            "the contemporary name of this square?"
        )
        bot.send_message(call.message.chat.id, text)
        user_data[call.message.chat.id] = "waiting_for_message"
    elif call.data == 'put1':
        text = (
            'Take a selfie so that the wooden facade of the former Administration of the Altai Mining District is '
            'visible in the background. This wooden building was built in 1898 in the classical style, with its love '
            'for symmetry, strict geometric forms, and light pilasters dividing the facade.'
        )
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton(text="I'm here!", callback_data="put0"))
        ...
        text = 'Hint: Polzunova St. 42, Mountain Pharmacy'
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton(text="I'm here!", callback_data="pol"))
        bot.send_message(call.message.chat.id, text, reply_markup=markup)
    elif call.data == 'pol':
        text = (
...
def handle_message(message):
    ...
    if user_data.get(chat_id) == "waiting_for_message":
        if (message.text.lower()).strip() == "sovetov square":
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton(text="Let's go!", callback_data="put1"))
    ...
    elif user_data.get(chat_id) == "waiting_for_message_1":
        ...
        if (message.text.lower()).strip() == "museum":
            markup = types.InlineKeyboardMarkup()
